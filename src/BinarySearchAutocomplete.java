...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PriorityQueue<Term> newque = new PriorityQueue<>(Comparator.comparing(Term :: getWeight));

		for (Term every : Arrays.copyOfRange(myTerms, first, last+1)) {
			if (newque.size() < k) 
			{newque.add(every);
			} 
			else if (every.getWeight() > newque.peek().getWeight()) 
			{newque.remove();
				newque.add(every);
			}
		}

		LinkedList<Term> flist = new LinkedList<>();
		for (int x = 0; x < Math.min(newque.size(), k); x++) {
			flist.addFirst(newque.remove());
		}
		return flist;		
	}
