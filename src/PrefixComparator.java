...
public class    PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        ...
        int no_of_compare = Math.min(Math.min(v.getWord().length(), w.getWord().length()), myPrefixSize);
        for (int x=0; x < no_of_compare; x++){
            if (w.getWord().charAt(x) > v.getWord().charAt(x)) {
                return -1;
            }
            else if (w.getWord().charAt(x) < v.getWord().charAt(x)) {
                return 1;
            }
        }
        if (myPrefixSize > no_of_compare){
            if (w.getWord().length() > v.getWord().length()) {
                return -1;
            }
            else if (w.getWord().length() < v.getWord().length()) {
                return 1;
            }
        }
        return 0;
